...
public class FantasyFootball {
	private final static int MAX_RECORD_LENGTH = 71;
	private final static int RECORD_COUNT = 20;
	...
	public static String padLeft(String str, int x, char c){
		String padding = new String(new char[x-str.length()]).replace('\0', c);
	...
	public static void main(String[] args) {		
		String cmd = "";
		File data = new File(System.getProperty("user.dir") + "\\datafiles\\lab02\\data.txt");
		...
		try {
			...
			JOptionPane.showMessageDialog(null, "There was a problem opening the file for writing: " + e.getMessage(),
		...
		if (storeLength < ((MAX_RECORD_LENGTH + 2) * RECORD_COUNT)) {
			for (int i = 0; i < 20; ++i) {
				try {
					...
					JOptionPane.showMessageDialog(null, "There was a problem writing to the file: " + e.getMessage(),
							"Uh oh", JOptionPane.ERROR_MESSAGE);
				}
			}
		}
		...
		while (cmd.compareToIgnoreCase("end") != 0) {
			cmd = JOptionPane.showInputDialog(null, "Please enter a command: new, old, or end", "Fantasy Football",
...
//				while (true) {
//					try {
//						id = JOptionPane.showInputDialog("Please enter an ID between 1 and 20, inclusively.");
...
//						}
//					} catch (Exception e) {
//						JOptionPane.showMessageDialog(null, "There was a problem with your entry: " + e.getMessage(), "Uh oh",
...
//				while (true) {
//					try {
//						name = JOptionPane.showInputDialog("Please enter the player's name (26 char max).");
//						if (name == null) {
//							System.exit(0);
//						} else if (name.length() < 1 || name.length() > 26) {
//							throw new Exception("Name must be between 1 and 26 characters in length.");
//						} else {
...
//				while (true) {
//					try {
//						team = JOptionPane.showInputDialog("Please enter the player's team name (26 char max).");
//						if (team == null) {
//							System.exit(0);
//						} else if (team.length() < 1 || team.length() > 26) {
//							throw new Exception("Team name must be between 1 and 26 characters in length.");
//						} else {
...
//					try {
//						skill = JOptionPane
//								.showInputDialog("Please enter the player's skill level between 0 and 99, inclusively.");
//						if (skill == null) {
//							System.exit(0);
//						}
...
//				}
				...
				while (true) {
					try {
						date = JOptionPane
								.showInputDialog("Please enter the player's draft date (format: YYYYMMDD, e.g. \"20150710\").");
						if (date == null) {
							System.exit(0);
						...
						String newDate = Integer.toString(cal.get(Calendar.YEAR))
								+ padZeros(Integer.toString(cal.get(Calendar.MONTH)), 2)
								+ padZeros(Integer.toString(cal.get(Calendar.DAY_OF_MONTH)), 2);
						if (JOptionPane.showConfirmDialog(null, "Did you mean " + newDate + "?", "Confirm date",
